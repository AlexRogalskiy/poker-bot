...
public class Extrapolator {
    ...
    /**
     * Provide the convergence point for two lines expressed as data points The
     * lists must contain more than 1 point each or you are asking for
     * convergence of a single point with another which is impossible If the
     * lines are parallel, or already converge, then this cannot be done It is
     * assumed that convergence is expected at some X > the largest X in the
     * lists
     *
     ...
     */
    public static DataPoint getConvergenceOf(List<DataPoint> list1,
        ...
        if (intersection == null || areDivergent(lastPointList1,
            intersection)) {
            return null;
        }
    ...
    public static Gradient getGradient(List<DataPoint> data) {
        ...
        Gradient currentGradient = Gradient
            .createFrom(data.get(0), data.get(1));
        for (int i = 1; i < data.size() - 1; i++) {
            Gradient nextGradient = Gradient
                .createFrom(data.get(i), data.get(i + 1));
            ...
            currentGradient = currentGradient
                .averageWith(nextGradient, data.get(i + 1));
        }
        return currentGradient;
    }

    public static DataPoint getConvergenceWithY(List<DataPoint> data,
        double y) {
    ...
    public static DataPoint getConvergenceWithX(List<DataPoint> data,
        double x) {
        Gradient gradient = getGradient(data);
        if (gradient == null) {
            return null;
