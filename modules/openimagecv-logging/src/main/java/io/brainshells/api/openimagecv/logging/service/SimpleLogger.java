...
/**
 * <p>
 * Simple implementation of {@link Logger} that sends all enabled impl
 * messages, for all defined loggers, to the console ({@code System.err}). The
 * following system properties are supported to configure the behavior of this
 * impl:
 * </p>
 * <p>
 * <ul>
...
public class SimpleLogger extends MarkerIgnoringBase {
    ...
    SimpleLogger(String name) {
        ...
        if (levelString != null) {
            this.rootLevel = SimpleLoggerConfiguration
     ...
     *
     * @param level   One of the LOG_LEVEL_XXX constants defining the impl
     *                level
    ...
    private void log(int level, String message, Throwable t) {
        ...
        if (CONFIG_PARAMS.showThreadName) {
            String threadName =
                CONFIG_PARAMS.disableColor ? LogUtils.getThreadPadding()
                    : LogUtils.getColorThreadPadding();
        ...
        if (CONFIG_PARAMS.showShortLogName) {
            if (shortLogName == null) {
                shortLogName =
                    CONFIG_PARAMS.disableColor ? LogUtils.getShortName(name)
                        : LogUtils.getColorShortName(name);
            }
    ...
    private void write(StringBuffer buf, Throwable t) {
        if (CONFIG_PARAMS.outputChoice.outputChoiceType
            == OutputChoice.OutputChoiceType.FILE) {
            ...
            PrintStream targetStream = CONFIG_PARAMS.outputChoice
     ...
     * level TRACE according to the format outlined above.
     */
    @Override
    ...
    /**
     * Perform single parameter substitution before logging the message of
     * level DEBUG according to the format outlined above.
     */
    @Override
    ...
    /**
     * Perform single parameter substitution before logging the message of
     * level INFO according to the format outlined above.
     */
    @Override
    ...
    /**
     * Perform single parameter substitution before logging the message of
     * level WARN according to the format outlined above.
     */
    @Override
    ...
    /**
     * Perform single parameter substitution before logging the message of
     * level ERROR according to the format outlined above.
     */
    @Override
    ...
    public void log(LoggingEvent event) {
        ...
        FormattingTuple tp = MessageFormatter
            .arrayFormat(event.getMessage(), event.getArgumentArray(),
                event.getThrowable());
